...
var getEnvVersion = function() {
  var envVersion = process.env.NG_VERSION

  if (envVersion) {
    var tag = envVersion.trim();
    var version = semver.parse(tag);

    if (version && checkBranchPattern(version.version, currentPackage.branchPattern)) {
      version.codeName = RegExp(`${version.version}(.*)$`).exec(tag)[1]
...
exports.currentVersion = getEnvVersion() || getTaggedVersion() || getSnapshotVersion();
